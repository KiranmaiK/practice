...
public class EmployeeController {
	...
	public String saveEmployee(@ModelAttribute("employeeForm") EmployeeForm employeeForm, ModelMap model) {
		/*
		 * try { employeeService.saveEmployee(employee.getFirstName(),
		 * employee.getLastName(), String.valueOf(employee.getAge()));
		 * List<Employee> employees = employeeService.getEmployees();
		 * model.addAttribute("employees", employees);
		 * 
		 * Employee employee = new Employee();
		 * model.addAttribute("employeeForm", employee); } catch (Exception e) {
		 * e.printStackTrace(); }
		 * 
		 * return "employee-add";
		 */
